...
#include "commons/CommonsUse.h"
#include "commons/CustomMath.h"
#include "Tp1Manager.h"
...
void _checkOptSelected(int iOptSlct, int *pINum1, int *pINum2, int *pResults) {
	...
	case 1:
		...
		*pINum1 = cm_getPositiveInteger(0);
		break;
	case 2:
		printf("--2-Ingresar Segundo operando--\n");
		*pINum2 = cm_getPositiveInteger(0);
		break;
	case 3:
...
void _calculateAllOPerations(int iNum1, int iNum2, int *pResults) {
	*(pResults + 0) = cmt_addition(iNum1, iNum2);  //suma en posicion 0
	*(pResults + 1) = cmt_subtraction(iNum1, iNum2); //resta en posicion 1
	*(pResults + 2) = cmt_multiplication(iNum1, iNum2); //multiplicacion en posicion 2
	*(pResults + 3) = cmt_division(iNum1, iNum2); //division en posicion 3
	*(pResults + 4) = cmt_factorial(iNum1);  //factorial 1er num en posicion 4
	*(pResults + 5) = cmt_factorial(iNum2); //factorial 2do num en posicion 5
}
