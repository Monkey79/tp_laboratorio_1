...
int ae_updateEmployee(Employee *list, int len, Employee empl) {
	int status = OK;
	int i = ae_findEmployeeById(list, len, empl.id);
...
int ae_removeEmployee(Employee *list, int len, int id) {
	int status = OK;
	int i = ae_findEmployeeById(list, len, id);
	if (i != -1) {
		(list+i)->isEmpty = TRUE;
		(list+i)->id = 0;
	} else {
...
int ae_printEmployees(Employee *list, int len) {
	...
	for (int i = 0; i < len - 1; i++) {
		if (!(list+i)->isEmpty) {
			printf ("%d %*s %*s %*.2f %*d\n", (list+i)->id,15,(list+i)->name,15,(list+i)->lastName,10,(list+i)->salary,5,(list+i)->sector);
		}
	}
	return 0;
}
...
int _getFreeEmplIndx(Employee *list, int len) {
	...
	int emptyIndx = -1;
	do {
		notEmpty = (list+i)->isEmpty == FALSE;
		i++;
	} while (notEmpty && i < len);
	emptyIndx = (notEmpty) ? emptyIndx : (i - 1);
	return emptyIndx;
}
